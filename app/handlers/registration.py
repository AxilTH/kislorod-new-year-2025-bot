...
async def handler_start(message: Message, state: FSMContext):
   ...
   '''
      ...
      ☃️ За окном тихо падает снег, вы чувствуете аромат свежих мандаринов, кругом царит атмосфера ожидания  чуда… А у нас для тебя есть свой, особенный календарь приятных сюрпризов на весь этот уютный месяц!

      🧣 Как устроен наш бот:
      1. <b>Каждый день открывай новое послание.</b> Пусть эта ежедневная приятность подскажет, как сделать день чуть светлее.
      2. <b>Заглядывай в расписание мероприятий на декабрь.</b> Да-да, теперь расписание можно посмотреть в телеграмме, чтобы ни одна встреча с друзьями, ни один тёплый вечер не прошёл мимо!
      3. <b>Выполняй ежедневные задания.</b> Они помогут создать праздничное настроение, а за старания ты получишь волшебные снежинки-баллы.
      4. <b>Приходи на наши мероприятия.</b> За участие в создании общего праздника мы начислим тебе снежинок! (снежинки за посещение мероприятий идут отдельно от БРС отряда)
...
async def handler_register_code(callback: CallbackQuery, state: FSMContext):
   ...
   text = textwrap.dedent(
   '''
      🔐 Регистрация в боте <b>"Новогодняя игра от СПО «Кислород»"</b>
...
async def handler_register(message: Message, state: FSMContext):
   ...
   success_text = textwrap.dedent(f'''
      ...
      Тебя ждут уютные вечера, тёплые встречи и много-много тайн и загадок! 
      Давай вместе создадим эту новогоднюю сказку!
   ''')
